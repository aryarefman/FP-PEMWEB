...
export abstract class SlidingPuzzleService {
    ...
    static async createSlidingPuzzle(data: ICreateSlidingPuzzle, user_id: string) {
    ...
    static async updateSlidingPuzzle(
        ...
        const newImagePaths: string[] = [thumbnailImagePath, puzzleImagePath];
    ...
    static async deleteSlidingPuzzle(game_id: string, user_id: string, user_role: ROLE) {
        const game = await prisma.games.findUnique({
            where: { id: game_id },
            select: {
                id: true,
